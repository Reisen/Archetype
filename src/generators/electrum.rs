//! Generate wallet seeds for Electrum.
//!
...
pub fn create_seed(url: Url, mut gen: SecretGenerator) -> Result<()> {
    ...
    // In Electrum, it loops twice. First to find a number that has enough
    // entropy (though the code is a little wonky...), and another to test
    // the seed type. As we need to discover a seed that meets these
    // criteria, we can do it in just one loop. To start with, we need a number
    // with 132 bits of entropy, so at least 33 bytes.
    //
    // Magical Constants:
    ...
    loop {
        ...
        // Bit fidling to get a 132 bit number out of our bytes.
        let mut number = u128::from_be_bytes(slice);

        // Electrum will reject anything that doesn't generate enough words
        // through this mechanism. I hate this, it basically removes
        // 2^31~ possibilities for seeds. This can't be right? Let's make
        // Electrum happy though...
        if number < 2u128.pow(132 - 11) {
            continue;
        }
        ...
        while number > 0 {
            let index = number % 2048;
            number /= 2048;
            words.push(WORDLIST[index as usize]);
        }

        // Confirm Prefix is as expected
        let mut hmac = Hmac::<Sha512>::new_varkey(b"Seed version").unwrap();
        let line = words.join(" ");
